...
description: 'A short note on how to clean up images and other files once they are no longer needed in a quarto document'
date: '2023-12-15'
...
To clean up the unused images, I ran these zsh commands from inside my quarto project:
...
2. Extract all the relative paths to files in `images/` from `slides.qmd` and move these files into the temporary folder
3. Remove the old `images/` folder that now only has unused images in it.
4. Rename the temporary folder to `images/`

### Requirements & Caveats

The command in lines 2-5 requires GNU grep which supports Perl Compatible Regular Expressions, but by default macOS has BSD grep pre-installed. You can install GNU grep using Homebrew via: `brew install grep`. Also, I asked chatGPT to generate the regex for me, so no guarantees on performance or correctness. It worked for me in the projects I was cleaning up but that's all I can say.
