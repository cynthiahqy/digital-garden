---
title: Custom Quarto Formats based on existing format-specific templates
description:  Tips on rendering Quarto files into existing format templates -- with examples using MS Word reference documents and Typst templates.
date: 2026-02-26
...
3. Set up the Quarto `_extensions/` folder with the relevant skeleton files^[See format guides in the Quarto docs (e.g. for [Typst](https://quarto.org/docs/output-formats/typst-custom.html#advanced-customization)) or the source code of existing [custom format extensions](https://quarto.org/docs/extensions/listing-formats.html) for reference].
...
::: {.callout-tip collapse="true"}
## Tip: Check Typst Package Manifests
An aside on Typst packages^[Typst Packages README: <https://github.com/typst/packages/blob/main/README.md>] -- all Typst packages have a manifest in the file `typst.toml` that looks something like (truncated for clarity):
...
- **Understand Quarto partials**: Famililarise yourself with how [Quarto template partials work](https://quarto.org/docs/journals/templates.html#template-partials). Looking at the source code of the templates will show you what Quarto is injecting alongside the format template you wrote when rendering your QMD file. If you use partials rather than replacing Quarto's base format templates completely, you can get conflicting or duplicate injections.
  - For example, I spent some time debugging multiple bibliographies not supported error, that was due to both Quarto and the Typst IEEE TVCG package adding `bibliography()` calls. The relevant partial was [`biblio.typ`](https://github.com/quarto-dev/quarto-cli/blob/main/src/resources/formats/typst/pandoc/quarto/biblio.typ), which was called by the main Quarto Typst template [`template.typ`](https://github.com/quarto-dev/quarto-cli/blob/main/src/resources/formats/typst/pandoc/quarto/template.typ). To get around this, I completely replaced the main Quarto Typst template with one I created myself based on the template in the Typst package I was adapting.
- **Use Intermediate output for debugging**: Set `keep-*:` to true in your example QMD, and the render that intermediate output to debug your template. The intermediate file will show you how Quarto is combining content from your QMD using the Pandoc template you provided and any applicable partials.
  - For example, I copied the intermediate `.typ` into a Typst universe project to get better hints and error messages. You could do the same with `.tex` files and debug them on Overleaf. Make sure to transfer any fixes back into your local source files.
- **Injecting raw cross-references**: If you create a special figure type (e.g. a teaser image), using a Pandoc template, you won't be able to cross-references this using standard Quarto `@fig-...` syntax. This is because Quarto doesn't know about figures or things that are directly constructed via the template. Instead, you'll need to directly use the syntax of your target format (e.g. Typst or LaTeX) to insert the cross-reference.
  - For example, if you specific a path to a teaser image in the YAML header, that gets passed to the template directly, then Quarto won't know about the teaser figure, and you can't use `@fig-teaser` to cross-reference it. In this case, you'll want to revert to raw LaTeX commands for cross-refs in your plain text QMD file. To do this, you can enclose the LaTeX command using an inline code chunk like ``` `Figure~\ref{fig:teaser}`{=latex} ```, just like the stand-alone Typst raw content shown above.
- **Specifying alternative engines:** It turns out that the IEEE TVCG Typst package required a more recent version of Typst than what Quarto bundles. There's a thread about this type of lagging in the [quarto-dev discussions](https://github.com/orgs/quarto-dev/discussions/11142). Long story short, the Quarto dev team prioritises stability, but you can specify non-standard configurations via the `QUARTO_TYPST` global variable.